...
scyclone_trained_model_path = "./output/scyclone/train/iteration99999/generator_A2B_trained_model_cpu.pth"
#結果を出力するためのディレクトリ
...
for i, audio_filepath in enumerate(audio_filepath_list, 0):
	...
	input_waveform = waveform.squeeze(dim=0)
	...
	output_waveform = torchaudio.transforms.GriffinLim(n_fft=254, n_iter=256, hop_length=128)(output_spectrogram)[None,...]
	#ファイルに保存
	save_dir = os.path.join(output_dir, f"result_{i}")
	os.makedirs(save_dir, exist_ok=True)#出力用ディレクトリがなければ作る
	torchaudio.save(os.path.join(save_dir, os.path.basename(audio_filepath)), waveform, sample_rate=16000)
